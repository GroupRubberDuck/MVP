...
def create_mock_validation_error() -> ValidationError:
    ...
    return ValidationError.from_exception_data(
        "GetRequirementEvaluationDetailCommand", line_errors=[]
    )
...
class TestFlaskRequirementEvaluationDetail:
    @patch(
        "adapters.inbound.evaluation.evaluation_detail.flask_requirement_evaluation_detail_controller.render_template"
    )
    ...
    @patch(
        "adapters.inbound.evaluation.evaluation_detail.flask_requirement_evaluation_detail_controller.GetRequirementEvaluationDetailCommand"
    )
    ...
    def test_ritorna_404_su_fallimento_use_case(
        ...
        mock_use_case.get_evaluation_detail.side_effect = (
            GetRequirementEvaluationDetailFailure("Non trovato")
        )
