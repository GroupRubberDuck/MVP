...
def create_mock_validation_error() -> ValidationError:
    ...
    return ValidationError.from_exception_data(
        "GetAssetEvaluationDetailCommand", line_errors=[]
    )
...
class TestFlaskAssetEvaluationDetailController:
    @patch(
        "adapters.inbound.evaluation.evaluation_detail.flask_asset_evaluation_detail_controller.render_template"
    )
    def test_ritorna_200_e_renderizza_dto_su_successo(
        self, mock_render_template, app_and_client
    ):
        """
        ...
        mock_asset_detail.asset_type = (
            getattr(AssetType, list(AssetType.__members__.keys())[0])
    ...
    @patch(
        "adapters.inbound.evaluation.evaluation_detail.flask_asset_evaluation_detail_controller.GetAssetEvaluationDetailCommand"
    )
    def test_ritorna_400_su_errore_di_validazione(
        self, mock_command_class, mock_render_template, app_and_client
    ):
        """
    ...
    def test_ritorna_404_su_fallimento_use_case(
        self, mock_render_template, app_and_client
    ):
        """
        ...
        mock_use_case.get_asset.side_effect = GetAssetDetailFailure(
            "Asset a1 non trovato"
        )
