...
def command():
    return DeleteAssetCommand(
        session_id="SESSION-123", device_id="DEV-001", asset_id="ASSET-456"
    )
...
class TestDeleteAssetSuccess:
    def test_deletes_asset_and_saves_session_successfully(
        self,
        service,
        mock_get_evaluation_session_port,
        mock_save_evaluation_session_port,
        command,
    ):
        """
        ...
        mock_get_evaluation_session_port.get_evaluation_session.return_value = (
            mock_session
        )

        service.delete_asset(command)

        mock_get_evaluation_session_port.get_evaluation_session.assert_called_once_with(
            "SESSION-123"
        )

        mock_session.device.remove_asset.assert_called_once_with("ASSET-456")

        mock_save_evaluation_session_port.save_evaluation_session.assert_called_once_with(
            mock_session
        )
...
class TestDeleteAssetFailures:
    def test_raises_failure_when_session_not_found(
        ...
        """
        mock_get_evaluation_session_port.get_evaluation_session.side_effect = (
