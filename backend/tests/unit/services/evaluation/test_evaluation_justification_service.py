...
def make_service():
    ...
    service = InsertJustificationService(
        get_evaluation_session_port=mock_get, save_evaluation_session_port=mock_save
    )
...
class TestInsertJustificationService:
    ...
    def test_salvataggio_avviene_dopo_insert(self):
        ...
        mock_asset.set_justification.side_effect = lambda *args, **kwargs: (
            call_order.append("insert")
        )
        mock_save.save_evaluation_session.side_effect = lambda _: call_order.append(
            "save"
        )
