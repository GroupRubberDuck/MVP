...
def requirement_with_tree(simple_tree) -> Requirement:
    return Requirement(
        ...
        description="Verifica accessi",
        target_description="Sistema login",
...
class TestRequirementEvaluation:
    ...
    def test_fail(self, requirement_with_tree):
        ...
        """
        answer = AssetEvidence(
            requirement_id="REQ-001", node_choices=MappingProxyType({"n1": False})
        )
...
class TestRequirementDependencies:
    @pytest.mark.parametrize(
        "dependency_states, expected",
        [
            # Tutte PASS → valutazione normale
            (
                (("DEP-1", EvaluationState.PASS), ("DEP-2", EvaluationState.PASS)),
                EvaluationState.PASS,
            ),
            # Una FAIL → bloccato con FAIL
            (
                (("DEP-1", EvaluationState.PASS), ("DEP-2", EvaluationState.FAIL)),
                EvaluationState.FAIL,
            ),
            # Una PENDING → bloccato con PENDING
            (
                (("DEP-1", EvaluationState.PASS), ("DEP-2", EvaluationState.PENDING)),
                EvaluationState.PENDING,
            ),
            # Una NA → bloccato con NA
            (
                (("DEP-1", EvaluationState.PASS), ("DEP-2", EvaluationState.NA)),
                EvaluationState.NA,
            ),
        ],
    )
    def test_dependency_blocking(
        self, requirement_with_tree, dependency_states, expected
    ):
        """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                (("D1", EvaluationState.PENDING), ("D2", EvaluationState.FAIL)),
                EvaluationState.FAIL,
            ),
            # FAIL ha priorità su NA
            (
                (("D1", EvaluationState.NA), ("D2", EvaluationState.FAIL)),
                EvaluationState.FAIL,
            ),
            # PENDING ha priorità su NA
            (
                (("D1", EvaluationState.NA), ("D2", EvaluationState.PENDING)),
                EvaluationState.PENDING,
            ),
        ],
    )
