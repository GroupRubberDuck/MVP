    ...
    def open_evaluation_session(self, command: OpenEvaluationSessionCommand) -> str:
        if not self._session_coordinator.can_open_session():
            raise OpenEvaluationSessionFailure(
                "Non è possibile aprire una nuova sessione di valutazione. Esiste già una sessione attiva."
            )
        ...
        try:
            session = self._create_session_port.create_evaluation_session(
                standard=standard, device=device
            )
            return session.session_id
        except EvaluationSessionOpenError as e:
            raise OpenEvaluationSessionFailure(
                f"Errore durante l'apertura della sessione di valutazione: {str(e)}"
            ) from e
