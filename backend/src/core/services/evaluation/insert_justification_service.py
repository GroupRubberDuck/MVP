...
class InsertJustificationService(InsertJustificationUseCase):
    ...
    def insert_justification(self, command: InsertJustificationCommand) -> None:
        ...
        try:
            session = self.get_evaluation_session_port.get_evaluation_session(
                command.session_id
            )
        except EvaluationSessionNotFoundError as e:
            raise InsertJustificationFailure(
                f"Errore durante il recupero della sessione: {str(e)}"
            )
        ...
        try:
            ...
            raise InsertJustificationFailure(
                f"Errore durante il recupero dell'asset: {str(e)}"
            )
        ...
        try:
            ...
            raise InsertJustificationFailure(
                f"Errore durante il salvataggio della sessione: {str(e)}"
            )
