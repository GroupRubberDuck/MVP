...
class FlaskEvaluateDecisionNodeController(FlaskController):
    def __init__(
        self, evaluate_decision_node_use_case: EvaluateDecisionNodeUseCase
    ) -> None:
    ...
    def register_routes(self, blueprint: Blueprint) -> None:
        ...
        def insert_decision_node_evaluation(
            session_id: str, device_id: str, asset_id: str, req_id: str
        ) -> ResponseReturnValue:
            ...
            try:
                self._evaluate_decision_node_use_case.evaluate_node(command=command)
