...
class FlaskDeviceEvaluationDetailController(FlaskController):
    ...
    def register_routes(self, blueprint: Blueprint) -> None:
        ...
        @blueprint.route(
            "/dashboard/sessions/<session_id>/devices/<device_id>", methods=["GET"]
        )
        def get_device_evaluation_detail(
            ...
            try:
                device_eval_detail = (
                    self._get_device_ev_detail_use_case.get_device_evaluation_detail(
                        command
                    )
                )
            except GetEvaluationDetailFailure as e:
                return render_template(
                    "errors/404.html", message=f"Risorsa non trovata: {e}"
                ), 404

            dto = self._make_dto(device_eval_detail)
            return render_template(
                "layouts/dashboard/dashboard.html", device_detail=dto.model_dump()
            ), 200
