...
class FlaskAssetEvaluationDetailController(FlaskController):
        ...
        def register_routes(self, blueprint: Blueprint) -> None:
                @blueprint.route("/session/<session_id>/devices/<device_id>/assets/<asset_id>",methods=["GET"])
                def get_asset_evaluation_detail(session_id:str, device_id:str, asset_id:str)-> ResponseReturnValue:
                        try:
                        ...
                        try:
                                asset=self._get_asset_ev_detail_use_case.get_asset(
                                        command
                                )
                        ...
                        try:
                                ...
                                return render_template(
                                    "errors/500.html", 
                                    message=f"si è verificato un errore nella costruzione della risposta: {e}"
                                ), 500

                        return render_template("layouts/asset_detail.html",asset=asset_dto),200