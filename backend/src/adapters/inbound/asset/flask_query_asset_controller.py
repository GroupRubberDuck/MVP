...
class FlaskQueryAssetController(FlaskController):
    def __init__(
        self, get_asset_anagraphic_use_case: GetAssetAnagraphicUseCase
    ) -> None:
    ...
    def register_routes(self, blueprint: Blueprint) -> None:
        ...
        @blueprint.route(
            "/sessions/<session_id>/devices/<device_id>/assets", methods=["GET"]
        )
        ...
        @blueprint.route(
            "/sessions/<session_id>/devices/<device_id>/assets/<asset_id>/edit",
            methods=["GET"],
        )
        def edit_asset_page(session_id: str, device_id: str, asset_id: str):

            command = GetAssetAnagraphicCommand(
                device_id=device_id, session_id=session_id, asset_id=asset_id
            )
